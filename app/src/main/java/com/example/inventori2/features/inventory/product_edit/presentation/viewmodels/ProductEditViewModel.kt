package com.example.inventori2.features.inventory.product_edit.presentation.viewmodels
...
import com.example.inventori2.features.inventory.product_edit.presentation.screens.ProductEditUIState
...
class ProductEditViewModel @Inject constructor(
    ...
    private val imageStorageManager: ImageStorageManager // Inyectado
) : ViewModel() {
        ...
        viewModelScope.launch {
            ...
            result.fold(
                onSuccess = { product ->
                    ...
                    _imageUri.value = product.imagenUri?.let { Uri.parse(it) } // Cargar imagen actual
    ...
    fun updateProduct() {
        ...
        _uiState.update { it.copy(isLoading = true, error = null) }
        viewModelScope.launch {
            // PROCESO DE IMAGEN: Copiar a local si es una URI temporal nueva
            val finalImagePath = _imageUri.value?.let { uri ->
                if (uri.toString().startsWith("content://")) {
                    imageStorageManager.saveImageLocally(uri)
                } else {
                    uri.toString() // Mantener la ruta local si ya era permanente
                }
            }
            ...
            val productEdit = ProductEdit(
                ...
                imagenUri = finalImagePath // Guardar la ruta procesada
            )
